...
use super::{configure, propose, approve, is_executable, mark_executed, get_config, MultisigError, Proposal};
use soroban_sdk::{testutils::Address as _, Address, BytesN, Env, Symbol, Vec, symbol_short};
...
fn test_configure_invalid() {
    ...
    // Threshold 0
    assert_eq!(configure(&env, signers.clone(), 0), Err(MultisigError::InvalidConfig));
    
    // Threshold > Signers
    assert_eq!(configure(&env, signers.clone(), 2), Err(MultisigError::InvalidConfig));
}
...
fn test_duplicate_approval_fails() {
    ...
    let id = propose(&env, &s1, symbol_short!("ACT"), BytesN::from_array(&env, &[0u8; 32])).unwrap();
...
fn test_large_signer_set() {
    ...
    let id = propose(&env, &proposer, symbol_short!("ACT"), BytesN::from_array(&env, &[0u8; 32])).unwrap();
