...
use teye_delegation::{executor::ExecutorInfo, DelegationContract, DelegationContractClient};

/// Setup environment with delegation contract
fn setup() -> (
    Env,
    DelegationContractClient<'static>,
    Address,
    soroban_sdk::Address,
) {
    let env = Env::default();
...
fn test_reputation_overflow_protection() {
    ...
    env.as_contract(&contract_id, || {
        env.storage()
            .persistent()
            .set(&(executors_key, executor.clone()), &info);
    });
...
fn test_slash_executor_underflow_saturation() {
    ...
    env.as_contract(&contract_id, || {
        env.storage()
            .persistent()
            .set(&(executors_key, executor.clone()), &low_info);
    });
