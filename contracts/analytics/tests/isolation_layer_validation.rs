...
fn test_time_bucket_key_validation() {
    ...
    for time_bucket in valid_time_buckets {
        let time_data = client.get_metric(
            &kind,
            &MetricDimensions {
                region: Some(region),
                age_band: Some(age_band),
        ...
        assert_eq!(
            time_data.count, 1,
            "Each time bucket should have its own data"
        );
    }
}
    ...
    for (i, dims_a) in test_cases.iter().enumerate() {
        for (j, dims_b) in test_cases.iter().enumerate() {
            if i != j {
                ...
                if dims_a.region != dims_b.region
                    || dims_a.age_band != dims_b.age_band
                    || dims_a.condition != dims_b.condition
                {
                    ...
                    assert_ne!(
                        dims_a, dims_b,
                        "Different test cases should have different dimensions"
                    );
                }
            }
        }
